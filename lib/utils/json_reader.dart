...
class JsonReader {
  ...
  JsonReader._(
    ...
    final isValidJson = json == null || json is Map || json is List || json is String || json is bool || json is num;
  ...
  List<Map<String, dynamic>> asListOfObjects() => asListOf<Map<String, dynamic>>();
...
Map<String, JsonReader> _getMap(
  ...
  if (v is Map && v.keys.every((dynamic k) => k is String)) {
    return v.map((dynamic k, dynamic v) => MapEntry(k as String, JsonReader._(v)));
  } else {
    throw ArgumentError('Illegal type: ${v.runtimeType}');
  }
}
